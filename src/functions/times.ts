...
solution
  .owner('cagataycali')
  .method('Array fill undefined and map over.')
  .fn((times, data) => {
    return new Array(times).fill(undefined).map(() => data);
  });

solution
  .owner('cagataycali')
  .method('Using Array.from().')
  .fn((times, data) => {
    return Array.from({ length: times }, () => data);
  });

solution
  .owner('cagataycali')
  .method('Using Array.from() another approach.')
  .fn((times, data) => {
    return Array.from(new Array(times), () => data);
  });
...
solution
  ...
  .fn((times, data) => {
    return [...new Array(times)].map(() => data);
  });

solution
  .owner('cagataycali')
  .method('Using dark magic.')
  .fn((times, data) => {
    return `?`
      .repeat(times)
      .split('')
