  ...
  async run(): Promise<BenchmarkReport> {
    ...
    this.solutions.forEach(solution => {
      const benchFunctions = this.benchParams.map(params => ({
        fn: solution.func.bind(null, ...params),
        case: `${this.name}(${JSON.stringify(params).slice(1, -1)})`,
      }));
    ...
    this.suite.on('complete', () => {
      ...
      this.suite.forEach((bench: any) => {
        ...
        table.push([owner, testCase, Math.round(bench.hz).toLocaleString()]);
      });

      const benchAverages = this.suite
        .map((bench: any) => {
          const [owner] = bench.name.split(' - ');
          const totalTests = this.suite.filter((suite: any) =>
            suite.name.startsWith(`${owner} -`)
          );
          const hz =
            totalTests.reduce((avg: number, c: any) => avg + c.hz, 0) /
            totalTests.length;

          return {
            name: owner,
            hz,
            text: Math.round(hz).toLocaleString(),
          };
        })
        .sort((a, b) => b.hz - a.hz);
      ...
      console.log(
        '🏆',
        `Fastest solution owner: ${benchAverages[0].name} with average ops/sec ${benchAverages[0].text}`
      );
      console.log('');
      console.log('');
      console.log('');
